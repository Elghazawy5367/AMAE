...
export async function submitImageToVideoJob(options) { 
  ...
  const response = await fetch(`${RUNWAY_API}/image_to_video`, {  method:  'POST',
    ...
    body: JSON.stringify({ 
      ...
      promptText:    options.motionPrompt ?? 'Subtle, smooth camera movement.  Professional.',
      model:         'gen3a_turbo', 
...
'campaigns/current/assets/video-jobs/runway-jobs.json') { 
  ...
  for (const spec of specs) { 
    try { 
      ...
      jobs.push({ outputFilename: spec.outputFilename, jobId, status: 'submitted', submittedAt:  new Date().toISOString() });
